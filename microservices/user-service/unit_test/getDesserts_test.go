...
func TestGetDesserts(t *testing.T) {
	...
	category := models2.Category{Name: "推荐"}
	database.DB.Create(&category)

	// 插入甜点数据（有category_id，但不含图片字段）
	dessert1 := models2.Dessert{Name: "巧克力蛋糕", Price: 19.99, Description: "浓郁巧克力", CategoryID: category.ID}
	dessert2 := models2.Dessert{Name: "草莓奶酪", Price: 15.50, Description: "清新草莓", CategoryID: category.ID}
